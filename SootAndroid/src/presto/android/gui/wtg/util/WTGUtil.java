...
public class WTGUtil {
  ...
  public boolean isSensorAcquireCall(Stmt s) {
    if (!s.containsInvokeExpr()) {
      return false;
    }
    SootMethod mtd = s.getInvokeExpr().getMethod();
    SootClass declz = mtd.getDeclaringClass();
    String mtdSig = mtd.getSignature();
    SootClass sensorManagerClass = Scene.v().getSootClass("android.hardware.SensorManager");
    if (declz.equals(sensorManagerClass)) {
      if (
              mtdSig.equals(MethodNames.registerSensorListener0Sig) ||
              mtdSig.equals(MethodNames.registerSensorListener1Sig) ||
              mtdSig.equals(MethodNames.registerSensorListener1Sig) ||
              mtdSig.equals(MethodNames.registerSensorListener3Sig)
         ) {
        return true;
      }
    }
  ...
  public boolean isSensorReleaseCall(Stmt s) {
    if (!s.containsInvokeExpr()) {
      return false;
    }
    SootMethod mtd = s.getInvokeExpr().getMethod();
    ...
    SootClass sensorManagerClass = Scene.v().getSootClass("android.hardware.SensorManager");
    if (declz.equals(sensorManagerClass)) {
      if (mtdSig.contains("unregister")) {
        Logger.verb("WTGUTIL" , "Signature " + mtdSig);
      }
      if (
              mtdSig.equals(MethodNames.unregisterSensorListener0Sig) ||
              mtdSig.equals(MethodNames.unregisterSensorListener1Sig)
         ) {
        return true;
      }
    }
