...
public class FixpointSolver {
  ...
  // Sensor stuff
  //NGetSensorNode to NSensorIdNode
  public Map<NOpNode, Set<NSensorIdNode>> reachingSensorIds;
  //NRegisterListenerOpNode to NGetSensorNode
  public Map<NOpNode, Set<NNode>> reachingParameterSensors;
  //NRegisterListenerOpNode to NObjectNode
  public Map<NOpNode, Set<NNode>> reachingSensorListeners;

  public Map<NOpNode, Set<NOpNode>> reachedParameterSensors;
  public Map<NOpNode, Set<NOpNode>> reachedSensorListeners;
  //public Map<NOpNode, Set<NGetSensorOpNode>>
  public Map<NSensorIdNode, Set<NRegisterListenerOpNode>> sensorAndRegisterEventHandlerMap;
  ...
  public void preSolveInit() {
    ...
    reachingSensorIds = Maps.newHashMap();
    reachingParameterSensors = Maps.newHashMap();
    reachingSensorListeners = Maps.newHashMap();
    reachedParameterSensors = Maps.newHashMap();
    reachedSensorListeners = Maps.newHashMap();
    sensorAndRegisterEventHandlerMap = Maps.newHashMap();
  }

  void computePathsFromViewProducerToViewConsumer() {
    viewAndListenerAsParameterAndReceiverReachability();
    sensorAndListenerAsParameterReachability();
    ...
    //Sensors
    reverseParameterSensorReachability();
    reverseSensorListenerReachability();
  ...
  void reverseParameterSensorReachability() {
    reverseReachbility(reachingParameterSensors, reachedParameterSensors);
  }

  void reverseSensorListenerReachability() {
    reverseReachbility(reachingSensorListeners, reachedSensorListeners);
  }
  ...
  /**
   * Look at all sensor IDs and see what operation nodes use them.
   * The result is saved in map reachingSensorIds
   */
  void sensorIdReachability() {
    for (NSensorIdNode sensorIdNode : flowgraph.allNSensorIdNodes.values()) {
      for (NNode target : graphUtil.reachableNodes(sensorIdNode)) {
        if (!(target instanceof NOpNode)) {
          continue;
        }
        NOpNode opNode = (NOpNode) target;
        if (opNode instanceof NGetSensorOpNode) {
          Set<NSensorIdNode> sensors = reachingSensorIds.get(opNode);
          if (sensors == null) {
            sensors = Sets.newHashSet();
            reachingSensorIds.put(opNode, sensors);
          }
          sensors.add(sensorIdNode);
        }
      }
    }
  }
  ...
  void sensorAndListenerAsParameterReachability() {
    for (NNode n : flowgraph.allNNodes) {
      if (n instanceof NGetSensorOpNode) {
        parameterSensorReachability(n);
      }

      if (n instanceof NObjectNode) {
        sensorListenerReachability(n);
      }
    }
  }

  void parameterSensorReachability(NNode source) {
    Set<NNode> reachables = graphUtil.reachableNodes(source);
    for (NNode target : reachables) {
      // Test if it is a register Sensor Listener Node
      if (target instanceof NRegisterListenerOpNode) {
        MultiMapUtil.addKeyAndHashSetElement(reachingParameterSensors, (NOpNode) target, source);
      }
    }
  }

  void sensorListenerReachability(NNode source) {
    Set<NNode> reachables = graphUtil.reachableNodes(source);
    for (NNode target : reachables) {
      if (target instanceof NRegisterListenerOpNode) {
        MultiMapUtil.addKeyAndHashSetElement(reachingSensorListeners, (NOpNode) target, source);
      }
    }
  }
...
//        vNode = flowgraph.inflNode(v.getSootClass());
  ...
  private void processInlineEventHandlers(NInflNode vNode, AndroidView androidView, NNode rootNode, NVarNode activityLocalForInflateOp) {
    ...
    System.out.println("Processing inline event handler");
    ...
    for(Map.Entry<EventType, String> eventTypeMethodName: inlineClickHandlers.entrySet()) {
      ...
      if (classesInHierarchyWithMethod.isEmpty()) {
        System.out.println("Unable to find method " + methodName + " in hierarchy of Activity " + activityClass);
        if (activityClass.declaresFieldByName(methodName)) {
          System.out.println("Field found with the same name " + methodName + " in Activity " + activityClass);
        }
        return;
      }
  ...
  void viewAndListenerPropagation() {
    while (true) {
      ...
      //Sensor Stuff
      for (NOpNode registerSensorListener : NOpNode.getNodes(NRegisterListenerOpNode.class)) {
        if (processSensorRegisterListener((NRegisterListenerOpNode)registerSensorListener)) {
          //changed = true;
          computePathsFromViewProducerToViewConsumer();
        }
      }
...
//                "SystemR.id.content used for non-activity object.");
  ...
  boolean processSensorRegisterListener(NRegisterListenerOpNode node) {
    boolean changed = false;
    Set<NNode> getSensorSet = reachingParameterSensors.get(node);
    if (getSensorSet == null || getSensorSet.isEmpty()) {
      return false;
    }

    Set<NSensorIdNode> reachedSensorIds = Sets.newHashSet();

    for (NNode getSensorNode : getSensorSet ) {
      if (!(getSensorNode instanceof NGetSensorOpNode)) {
        throw new RuntimeException("Unexpected node in getSensorSet " + getSensorNode.toString());
      }
      Set<NSensorIdNode> sensorIdSet = reachingSensorIds.get(getSensorNode);
      if (sensorIdSet != null && (!sensorIdSet.isEmpty())) {
        reachedSensorIds.addAll(sensorIdSet);
      }
    }

    if (reachedSensorIds.isEmpty()) {
      return false;
    }

    Set<NNode> listenerObjSet = reachingSensorListeners.get(node);
    if (listenerObjSet == null || listenerObjSet.isEmpty()) {
      return false;
    }

    for (NSensorIdNode sensorIdNode : reachedSensorIds) {
      for (NNode listenerNode : listenerObjSet) {
        NObjectNode listenerObject = (NObjectNode) listenerNode;
        addSensorAndEventHandlerRegistration(sensorIdNode, node);
        if (flowgraph.processSensorRegisterListenerOpNode(node, sensorIdNode, listenerObject)) {
          changed = true;
        }
      }
    }
  ...
  public void addSensorAndEventHandlerRegistration(NSensorIdNode sensorId,
                                                   NRegisterListenerOpNode registrationNode) {
    Set<NRegisterListenerOpNode> sensorRegistration = sensorAndRegisterEventHandlerMap.get
            (sensorId);
    if (sensorRegistration == null) {
      sensorRegistration = Sets.newHashSet();
      sensorAndRegisterEventHandlerMap.put(sensorId, sensorRegistration);
    }
    sensorRegistration.add(registrationNode);
  }
