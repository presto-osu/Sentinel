...
public class DFSGenericPathGenerator {
  ...
  public void doPathGenerationWithTarget(){
    ...
    for (WTGEdge initEdge : this.m_initEdges){
      ...
      Stack<NObjectNode> S = new Stack<NObjectNode>();
      if (initEdge == null) {
        Logger.verb("DEBUG", "Init edge is null");
        continue;
      }

      if (initEdge.getTargetNode() == null) {
        Logger.verb("DEBUG", "Init target edge edge is null");
        continue;
      }

      if (initEdge.getTargetNode().getWindow() == null) {
        Logger.verb("DEBUG", "Init edge target window is null");
        continue;
      }
  ...
  private boolean helperCheckPushPop(List<WTGEdge> P, int K) {
    int sum = 0;
    for (WTGEdge curEdge : P) {
      List<StackOperation> stackOps = curEdge.getStackOps();
      sum += stackOps.size();
    }
    if (sum > K)
      return false;
