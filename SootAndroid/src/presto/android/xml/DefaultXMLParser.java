    ...
    try {
      ...
      for (int i = 0; i < nodes.getLength(); ++i) {
        try {
          ...
          if ("activity-alias".equals(eleName)) {
            NamedNodeMap m = n.getAttributes();
            String cls = Helper.getClassName(
                    m.getNamedItem("android:targetActivity").getTextContent(), appPkg);
            String originName = Helper.getClassNameAlias(m.getNamedItem("android:name").getTextContent(),
                    appPkg);
            if (cls == null)
              continue;
            if (originName == null)
              continue;
            Logger.verb("ActivityAlias", "origin: " + originName + " alias: " + cls);
            if (isMainActivity(n)) {
              if (mainActivity == null) {
                mainActivity = Scene.v().getSootClass(cls);
              }
            }
            aliasActivityToOriginMap.put(originName, cls);
            if (!activities.contains(cls)) {
              activities.add(cls);
            }
            if (!aliasActivityMap.containsKey(cls)) {
              aliasActivityMap.put(cls, Lists.newArrayList());
            }
            aliasActivityMap.get(cls).add(originName);
          }
